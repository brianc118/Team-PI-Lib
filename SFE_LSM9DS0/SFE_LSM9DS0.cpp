...
Development environment specifics:
	IDE: Arduino 1.0.6 with Teensyduino 1.20
	Hardware Platform: Teensy 3.1 @ 96MHz
	LSM9DS0 Breakout Version: Adafruit
...
{
	SPIreadByte(gAddress, subAddress);
}
...
{
	SPIreadByte(xmAddress, subAddress);
}
...
void LSM9DS0::initSPI()
{
	...
	digitalWriteFast(gAddress, HIGH);
	pinMode(xmAddress, OUTPUT);
	digitalWriteFast(xmAddress, HIGH);
...
void LSM9DS0::SPIwriteByte(uint8_t csPin, uint8_t subAddress, uint8_t data)
{
	...
	digitalWriteFast(csPin, LOW); // Initiate communication
	...
	digitalWriteFast(csPin, HIGH); // Close communication
	SPI.endTransaction();
}
