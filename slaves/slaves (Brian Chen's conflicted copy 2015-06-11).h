...
public:
	...
	uint8_t txrx(uint8_t command){
		...
		in = SPI.transfer(command); // not really the command any more. Just to save space
		digitalWriteFast(cs, HIGH);
		SPI.endTransaction();
		return in;
	}
...
public:
	...
	uint8_t getTSOPAngleStrength(uint8_t &angle, uint8_t &strength){
		SPI.beginTransaction(SPISettings(4000000, MSBFIRST, SPI_MODE0));
		digitalWriteFast(cs, LOW);
		angle = SPI.transfer(SLAVE2_COMMANDS::TSOP_ANGLE_BYTE);
		delayMicroseconds(1);
		strength = SPI.transfer(SLAVE2_COMMANDS::TSOP_STRENGTH);
		digitalWriteFast(cs, HIGH);
		SPI.endTransaction();
	}
};
